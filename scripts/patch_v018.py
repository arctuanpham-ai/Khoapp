...
# Faster intentional pinch response.
s=s.replace('.zoomSpeed(0.006f)', '.zoomSpeed(0.018f)')

# Add custom gesture state.
...
# Replace touch controller with explicit Filament Manipulator control.
                    ...
                    MotionEvent.ACTION_DOWN -> {
                        ...
                        surfaceView.postDelayed({
                            if (dimMode && longPressArmed && !precisionHold) {
                                ...
                                setStatus("DIM • rê tâm ngắm • nhả để chốt điểm")
                            }
                        },300)
                        true
                    }
                ...
                when (event.actionMasked) {
                    ...
                    MotionEvent.ACTION_POINTER_DOWN -> {
                        if (event.pointerCount>=2) {
                            ...
                            cameraManipulator.grabBegin(navLastMidX.toInt(),navLastMidY.toInt(),true)
                    ...
                    MotionEvent.ACTION_MOVE -> {
                        if (navTwoFinger && event.pointerCount>=2) {
                            ...
                            // Dead-zone: ordinary two-finger jitter remains PAN, not zoom.
                            if (kotlin.math.abs(ratio-1f) < 0.025f) {
                                cameraManipulator.grabUpdate(mx.toInt(),my.toInt())
                                setStatus("PAN • kéo 2 ngón song song")
                            } else {
                                // Intentional pinch: substantially faster response than v0.1.7.
                                val delta=((ratio-1f)*42f).coerceIn(-2.2f,2.2f)
                                cameraManipulator.scroll(mx.toInt(),my.toInt(),delta)
                                navLastSpan=span
                                // Re-anchor pan after zoom so the next parallel move is stable.
                                cameraManipulator.grabEnd(); cameraManipulator.grabBegin(mx.toInt(),my.toInt(),true)
                                setStatus("ZOOM • pinch")
                            }
...
# Robust world unprojection: inverse(ViewProjection), with both common depth conventions.
...
rep2='''    private fun unproject(frag: FloatArray): DoubleArray? {
        ...
        val view=modelViewer.camera.getViewMatrix(null)
        val vp=mulMat4(proj,view)
        val inv=invert4(vp) ?: return null
        val nx=frag[0].toDouble()/surfaceView.width.toDouble()*2.0-1.0
        val ny=frag[1].toDouble()/surfaceView.height.toDouble()*2.0-1.0
        val zs=doubleArrayOf(frag[2].toDouble()*2.0-1.0, frag[2].toDouble())
        for (z in zs) {
            val q=mul4(inv,doubleArrayOf(nx,ny,z,1.0))
            if (kotlin.math.abs(q[3])>1e-9 && q.all { it.isFinite() }) {
                return doubleArrayOf(q[0]/q[3],q[1]/q[3],q[2]/q[3])
            }
        }
        return null
    }

    private fun mulMat4(a: DoubleArray,b: DoubleArray): DoubleArray {
        val r=DoubleArray(16)
        for (c in 0..3) for (row in 0..3) {
            var v=0.0
            for (k in 0..3) v += a[k*4+row]*b[c*4+k]
            r[c*4+row]=v
        }
        return r
    }
