...
@Entity(indices=[Index(value=["phone"],unique=true),Index("tier")]) data class CustomerEntity(@PrimaryKey val id:String,val phone:String,val name:String="",val tier:String="MEMBER",val points:Int=0,val totalSpend:Long=0,val visitCount:Int=0,val lastVisitAt:Long?=null,val active:Boolean=true,val tierManual:Boolean=false)
