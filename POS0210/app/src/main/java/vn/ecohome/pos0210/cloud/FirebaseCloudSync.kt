...
object FirebaseCloudSync {
    ...
    suspend fun syncNow(context:Context):Result<Unit> = runCatching{
        ...
        writeMaps(fs,root.collection("purchases"),dao.cloudPurchasesSnapshot().map{p->p.id to mapOf("id" to p.id,"purchasedAt" to p.purchasedAt,"total" to p.total,"note" to p.note,"status" to p.status,"expenseCategory" to p.expenseCategory,"paidByName" to p.paidByName)})
